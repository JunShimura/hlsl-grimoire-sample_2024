...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
	...
	ModelInitData teapotShadoweModelInitData;

	//シャドウマップ描画用のシェーダーを指定する
	teapotShadoweModelInitData.m_fxFilePath = "Assets/shader/sampleDrawShadowMap.fx";
	teapotShadoweModelInitData.m_tkmFilePath = "Assets/modelData/teapot.tkm";

	// 【注目】影用のパラメータを拡張定数バッファーに設定する
	teapotShadoweModelInitData.m_expandConstantBuffer = (void*)&sp;
	teapotShadoweModelInitData.m_expandConstantBufferSize = sizeof(sp);

	// ティーポットモデルを初期化する
	Model teapotShadowModel;
	teapotShadowModel.Init(teapotShadoweModelInitData);
